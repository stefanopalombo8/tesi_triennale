...
for snippet_model in models_to_do[200:]: # nomi sbagliati
    all_snippets = []

    snippet_model_for_best = snippet_model.replace("/", "_") 
    print(f"Processing model: {snippet_model}")

    snippet_model_corretto = trasforma_nome(snippet_model)
    print(f"Processing model corretto: {snippet_model_corretto}")
    # Pattern per il nome del modello (assicurati di escapare se necessario)
    pattern_model = re.compile(r"\b" + re.escape(snippet_model_corretto) + r"\b", flags=re.IGNORECASE)
    ...
    system_prompt = (
        ...
        f"Focus solely on identifying and returning the common patterns in the context of the model '{snippet_model_corretto}'.\n"
    ...
    files = data[snippet_model_for_best]
